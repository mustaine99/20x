	...
	$(document).ready(function(){
		...
		$('.main-links li a').on('click', function(e){
			...
			$('.submenu-links ul[data-place!="'+ place +'"]').css('display','none');
            $('.submenu-links ul[data-place!="'+ place +'"]').removeClass('current-area');
            
			$('.submenu-links ul[data-place="'+ place +'"]').css('display','block');
            $('.submenu-links ul[data-place="'+ place +'"]').addClass('current-area');
            
			$('.submenu-links').fadeIn(300);
    ...
    function setPoints(points) {
		...
		var boxText = document.createElement("div");
	        boxText.style.cssText = "padding: 20px; background:#cda760; border:1px solid #cda760;";
	        boxText.innerHTML = '<div class="infobox_title" style="padding-bottom:10px; margin-bottom:10px; border-bottom: 1px solid #dcc290; font-weight: bold; font-style: italic; font-size: 14px; line-height: 16px; color: #FFF; width:100%;">Title</div><p class="infobox_text" style="width:100%; color: #FFF; font-size: 13px; line-height: 14px;">This is a description</p>';
		var infoBoxOptions = {
			 content: boxText
			...
			,zIndex: null
			,boxStyle: { 
			  background: "url('" + template_uri + "/images/arrow-location.png') no-repeat 10px 0"
			  ,width: "280px"
			  ,padding: "6px 0 0 0"
			 }
			,closeBoxURL: template_uri + "/images/btn-close.png"
